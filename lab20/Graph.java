    ...
    public ArrayList<Integer> shortestPath(int startVertex, int endVertex) {
        ...
        while (fringe.size() != 0) {
            ...
            for (int i = 0; i < adjLists[curr].size(); i += 1) {
                ...
                if (distance.get(neigh) > (Integer) getEdge(curr, neigh).info() + distance.get(curr)) {
                    distance.put(neigh, (Integer) getEdge(curr, neigh).info() + distance.get(curr));
                    predecessor.put(neigh, curr);
                }
        ...
        while (pointer != startVertex) {
            pointer = predecessor.get(pointer);
            path.add(pointer);
        }
